...
def get_version():
    version_file = 'cvutils/version.py'
...
def get_extensions():
    ...
    if os.getenv('MMCV_WITH_TRT', '0') != '0':
        ...
        include_path = os.path.abspath('./cvutils/ops/csrc/common/cuda')
        include_trt_path = os.path.abspath('./cvutils/ops/csrc/tensorrt')
        include_dirs.append(include_path)
        include_dirs.append(include_trt_path)
        include_dirs.append(os.path.join(tensorrt_path, 'include'))
        include_dirs += include_paths(cuda=True)

        op_files = glob.glob('./cvutils/ops/csrc/tensorrt/plugins/*')
    ...
    if EXT_TYPE == 'parrots':
        ...
        define_macros = []
        include_dirs = []
        op_files = glob.glob('./cvutils/ops/csrc/pytorch/cuda/*.cu') +\
            glob.glob('./cvutils/ops/csrc/pytorch/cpu/*.cpp') +\
            glob.glob('./cvutils/ops/csrc/parrots/*.cpp')
        op_files.remove('./cvutils/ops/csrc/pytorch/cuda/iou3d_cuda.cu')
        include_dirs.append(os.path.abspath('./cvutils/ops/csrc/common'))
        include_dirs.append(os.path.abspath('./cvutils/ops/csrc/common/cuda'))
        cuda_args = os.getenv('MMCV_CUDA_ARGS')
        extra_compile_args = {
        ...
        # Since the PR (https://github.com/open-mmlab/cvutils/pull/1463) uses
        ...
        if is_rocm_pytorch or torch.cuda.is_available() or os.getenv(
            ...
            op_files = glob.glob('./cvutils/ops/csrc/pytorch/*.cpp') + \
                glob.glob('./cvutils/ops/csrc/pytorch/cpu/*.cpp') + \
                glob.glob('./cvutils/ops/csrc/pytorch/cuda/*.cu') + \
                glob.glob('./cvutils/ops/csrc/pytorch/cuda/*.cpp')
            extension = CUDAExtension
            include_dirs.append(os.path.abspath('./cvutils/ops/csrc/pytorch'))
            include_dirs.append(os.path.abspath('./cvutils/ops/csrc/common'))
            include_dirs.append(os.path.abspath('./cvutils/ops/csrc/common/cuda'))
            ...
            cvutils_mluops_version = get_mluops_version(
                './cvutils/ops/csrc/pytorch/mlu/mlu_common_helper.h')
            mlu_ops_path = os.getenv('MMCV_MLU_OPS_PATH')
            if mlu_ops_path:
                exists_mluops_version = get_mluops_version(
                    mlu_ops_path + '/bangc-ops/mlu_op.h')
                if exists_mluops_version != cvutils_mluops_version:
                    print('the version of mlu-ops provided is %s,'
                          ' while %s is needed.' %
                          (exists_mluops_version, cvutils_mluops_version))
                    exit()
                try:
                ...
                if not os.path.exists('mlu-ops'):
                    ...
                    mluops_url = 'https://github.com/Cambricon/mlu-ops/' + \
                        'archive/refs/tags/' + cvutils_mluops_version + '.zip'
            ...
            op_files = glob.glob('./cvutils/ops/csrc/pytorch/*.cpp') + \
                glob.glob('./cvutils/ops/csrc/pytorch/cpu/*.cpp') + \
                glob.glob('./cvutils/ops/csrc/pytorch/mlu/*.cpp') + \
                glob.glob('./cvutils/ops/csrc/common/mlu/*.mlu') + \
                glob.glob(
            ...
            include_dirs.append(os.path.abspath('./cvutils/ops/csrc/common'))
            include_dirs.append(os.path.abspath('./cvutils/ops/csrc/common/mlu'))
            ...
            op_files = glob.glob('./cvutils/ops/csrc/pytorch/*.cpp') + \
                glob.glob('./cvutils/ops/csrc/pytorch/cpu/*.cpp') + \
                glob.glob('./cvutils/ops/csrc/common/mps/*.mm') + \
                glob.glob('./cvutils/ops/csrc/pytorch/mps/*.mm')
            extension = CppExtension
            include_dirs.append(os.path.abspath('./cvutils/ops/csrc/common'))
            include_dirs.append(os.path.abspath('./cvutils/ops/csrc/common/mps'))
            ...
            op_files = glob.glob('./cvutils/ops/csrc/pytorch/*.cpp') + \
                glob.glob('./cvutils/ops/csrc/pytorch/cpu/*.cpp') + \
                glob.glob('./cvutils/ops/csrc/common/npu/*.cpp') + \
                glob.glob('./cvutils/ops/csrc/pytorch/npu/*.cpp')
            include_dirs.append(os.path.abspath('./cvutils/ops/csrc/common'))
            include_dirs.append(os.path.abspath('./cvutils/ops/csrc/common/npu'))
        else:
    ...
    if EXT_TYPE == 'pytorch' and os.getenv('MMCV_WITH_ORT', '0') != '0':
        ...
        include_path = os.path.abspath('./cvutils/ops/csrc/onnxruntime')
        include_dirs.append(include_path)
        include_dirs.append(os.path.join(ort_path, 'include'))

        op_files = glob.glob('./cvutils/ops/csrc/onnxruntime/cpu/*')
        if onnxruntime.get_device() == 'GPU' or os.getenv('FORCE_CUDA',
                                                          '0') == '1':
            define_macros += [('MMCV_WITH_CUDA', None)]
            cuda_args = os.getenv('MMCV_CUDA_ARGS')
            extra_compile_args['nvcc'] = [cuda_args] if cuda_args else []
            op_files += glob.glob('./cvutils/ops/csrc/onnxruntime/gpu/*')
            include_dirs += include_paths(cuda=True)
            library_dirs += library_paths(cuda=True)
        else:
...
setup(
    name='cvutils' if os.getenv('MMCV_WITH_OPS', '0') == '0' else 'cvutils-full',
    version=get_version(),
    ...
    url='https://github.com/open-mmlab/cvutils',
